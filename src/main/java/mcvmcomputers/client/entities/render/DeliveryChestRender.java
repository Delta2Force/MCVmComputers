...
public class DeliveryChestRender extends EntityRenderer<EntityDeliveryChest>{
	...
	public DeliveryChestRender(EntityRendererFactory.Context context) {
		...
		try {
			deliveryChestModel = new DeliveryChestModel(context.getPart(MODEL_DELIVERY_CHEST_MODEL));
		}catch(IOException e) {
			e.printStackTrace();
		}
	}
				...
				matrices.push();   
					...
					if(ClientMod.myOrder != null) {
						if(entity.getDeliveryUUID().equals(ClientMod.myOrder.orderUUID)) {
							switch(ClientMod.myOrder.currentStatus) {
							...
							render_this_is_not_your_chest(matrices, vertexConsumers, light);
						}
					}else {
						render_this_is_not_your_chest(matrices, vertexConsumers, light);
					}
