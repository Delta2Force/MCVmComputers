...
public class WallTVRender extends EntityRenderer<EntityWallTV>{
	public WallTVRender(EntityRendererFactory.Context dispatcher) {
		super(dispatcher);
	}
	...
	public void render(EntityWallTV entity, float yaw, float tickDelta, MatrixStack matrices,
		...
		MinecraftClient.getInstance().getItemRenderer().renderItem(new ItemStack(ItemList.ITEM_WALLTV), Mode.NONE, light, OverlayTexture.DEFAULT_UV, matrices, vertexConsumers, 0);
		matrices.pop();
		if(ClientMod.vmScreenTextures.containsKey(UUID.fromString(entity.getOwnerUUID()))) {
			...
			VertexConsumer vertexConsumer = vertexConsumers.getBuffer(RenderLayer.getTextSeeThrough(ClientMod.vmScreenTextures.get(UUID.fromString(entity.getOwnerUUID()))));
